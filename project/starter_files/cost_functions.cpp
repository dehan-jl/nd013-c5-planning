    ...
    for (size_t c = 0; c < n_circles && !collision; ++c) {
      ...
      for (auto obst : obstacles) {
        ...
        for (size_t c2 = 0; c2 < n_circles && !collision; ++c2) {
          ...
          // obstacle/actor
          double diff_x = circle_center_x - actor_center_x;
          double diff_y = circle_center_y - actor_center_y;
...
double close_to_main_goal_cost_spiral(const std::vector<PathPoint>& spiral,
  ...
  auto delta_x = spiral.back().x - main_goal.location.x;
  auto delta_y = spiral.back().y - main_goal.location.y;
  auto delta_z = spiral.back().z - main_goal.location.z;
